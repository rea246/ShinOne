...
N차원 공간(차원 수 = common.N_FEATURES)을 거친 격자로 이산화한 뒤,
데이터가 실제 밟은 고유 격자 집합을 비교한다.
    Coverage = |Set_sam ∩ Set_ref| / |Set_ref|

차원의 저주 대응
    * 축당 Bin 을 3~5개로 작게 유지 → 이산화를 거칠게.
    * (Bin^차원) 전체 격자 배열을 만들지 않고, '실제 점유된 격자' 만 set 으로 관리(Sparse).
...
from common import (
    ...
    fit_scaler, read_feature_matrix, parallel_reduce_reference, set_union,
)
...
# 격자화 유틸
# =============================================================================
# 모든 축이 동일한 내부 경계를 공유하므로, 전체 축을 하나의 배열 호출로 이산화한다.
_INNER_EDGES = np.linspace(0.0, 1.0, N_BINS + 1)[1:-1]   # 길이 N_BINS-1


def cells_of(block):
    """
    스케일링된 (n_rows, N_FEATURES) 블록 → 그 블록이 점유한 '고유 격자' 집합.

    - np.digitize 는 배열 전체에 적용되어 각 값을 0..N_BINS-1 로 이산화 (범위 밖 값도 안전).
    - 격자 좌표(N_FEATURES개 int16)를 .tobytes() 로 hashable key 화 → set 으로 중복 제거.
      (교집합 계산에는 동일성만 필요하므로 좌표를 tuple 로 복원할 필요 없음)
    """
    if block.shape[0] == 0:
        return set()
    idx = np.digitize(block, _INNER_EDGES).astype(np.int16)   # (n_rows, N_FEATURES)
...
# =============================================================================
def run():
    # (1) Sample 로드(위치 기준 열 선택) → (2) Sample 기준 스케일러 학습 → 변환
    sample = read_feature_matrix(SAMPLE_PATH, fmt="csv")
    scaler = fit_scaler(sample, SCALE_METHOD)
    set_sam = cells_of(scaler.transform(sample))
    print(f"[BinCount] 축당 Bin={N_BINS}, 차원={N_FEATURES} "
          f"(이론상 {N_BINS}^{N_FEATURES}={N_BINS**N_FEATURES:,} 격자), "
