"""
train_ver4_cov.py  —  train_ver2.py 로부터 다시 분기한 실험용 버전.

경위: train_ver3_conv.py 에서 Cov weight 0.5 + EMBEDDING_DIM 16(이후 9)을 같이 실험했는데,
dim 변경이 성능 저하/조기 plateau 및 (dim=9 에서 hidden_dim 8배수 정렬이 깨지며) compute
time 급증까지 겹쳐 원인이 뒤섞였다. USE_TORCH_COMPILE 까지 켜보니 오히려 200s 로 더
악화돼(배치마다 그래프 shape 이 달라 recompile 이 반복되는 등, PyG 연산이 이 워크로드엔
안 맞는 것으로 보임) train_ver3_conv.py 는 폐기한다.

train_ver4_cov.py 는 v2 에서 "Cov weight 만" 깨끗하게 다시 분리해서 본다:
  1. VICREG_WEIGHT_COV 0.1 → 0.5 (v3_conv 와 동일한 목표치, dim 은 안 건드림)
  2. best 체크포인트(ckpt_name) 저장 대상을 encoder 단독 → 전체 GAE(model.state_dict())로 변경
     (v3_conv 에서 이미 적용했던 것과 동일한 이유 — center_proj 포함 정확한 재현)
  3. EMBEDDING_DIM 은 8로 유지(v2 와 동일) — Cov weight 효과만 보기 위해 변수를 하나로 고정.
  RESUME_PATH/ckpt_name 도 v2/v3_conv 와 겹치지 않게 v4_cov 전용 파일명을 쓴다.
  (v3_conv 에서 했던 sync-제거 성능 패치, VAL_SPLIT_SEED 변경, torch.compile 토글은
  이 파일엔 가져오지 않았다 — v2 와 최대한 가까운 상태에서 Cov weight 하나만 비교하기 위함.
  필요하면 나중에 별도로 적용할 것.)
...
# v4_cov 에서는 8로 유지 — v3_conv 에서 dim 변경과 Cov weight 변경을 같이 해서 어느 쪽 때문에
# 지표가 움직였는지 구분이 안 됐다. 여기선 dim 을 v2 와 동일하게 고정해 Cov weight 효과만 본다.
EMBEDDING_DIM = 8
...
# v2 에서 0.1 로 ep180 raw vic_cov≈0.75 로 잔류 상관이 남아 0.5 로 상향(v3_conv 와 동일 목표치).
...
def train_one_epoch(
    ...
    model.train()
    # ── [성능 수정] 로깅용 누적을 GPU 텐서로 유지, epoch 끝에 .item() 1회만 호출.
    #   기존엔 배치마다 loss/recon/vi/vv/vc(5) + grad-norm(2) 총 ~7번 .item() 을 불렀는데,
    #   .item() 은 그 시점까지 큐잉된 GPU 커널이 전부 끝날 때까지 CPU 를 블로킹한다.
    #   그래프가 작아 커널이 매우 많고 짧아서(launch-overhead-bound), 배치 수가 많을수록
    #   (23.7M graphs / batch 2048 ≈ 11,600 배치/epoch) 이 sync 비용이 누적돼 t_step 을
    #   실제 연산량보다 크게 부풀린다. NaN/Inf loss 를 걸러 backward 를 스킵하는 체크만
...
def validate_epoch(
    ...
    # [성능 수정] train_one_epoch 와 동일한 이유로 GPU 텐서 누적 + epoch 끝 1회 sync.
    def _zero():
...
# ══════════════════════════════════════════════════════════════════
RESUME_PATH = "checkpoint_last_v4_cov.pt"   # 매 에포크 갱신되는 '이어하기'용 풀 체크포인트
...
def main():
    ...
    print(f"[train_ver4_cov] batch={BATCH_SIZE}  lr={lr:.2e} (sqrt-scaled from {base_lr:.0e}@{REFERENCE_BATCH})")
    ...
    patience          = 30
    epochs            = 1000
    ckpt_name         = f"best_gae_v4_cov_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pt"
    _save_thread      = None
    start_epoch       = 0
    ...
    if os.path.exists(RESUME_PATH):
        ...
        print(f"\n[train_ver4_cov] 신규 학습 시작 (RESUME_PATH 없음)")

    print(f"[train_ver4_cov] embedding_dim={EMBEDDING_DIM} | "
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            # v4_cov 는 best 시점의 전체 GAE(encoder+decoder+center_proj) 를 그대로 저장해
