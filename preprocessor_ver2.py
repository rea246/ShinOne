"""
preprocessor_ver2.py — preprocessor.py 로부터 분기, gauge_density feature 실험용.

preprocessor.py 대비 변경점:
  게이지(gauge) 1개당 DENSITY_WINDOW_UM(기본 10um) 정사각형 안의 폴리곤 면적비(0~1)를
  "게이지당 1번만" 계산해서, 그 게이지에 속한 모든 노드(hop0~hop3+ 전부)에 동일한 값으로
  broadcast 한다 — 노드별로 따로 계산하지 않는다(합의된 전제: 어느 정도 범위의 gauge 는
  같은 density 값을 공유해도 된다). x 가 [cx,cy,w,h] 4열 → [cx,cy,w,h,gauge_density] 5열로
  늘어난다. train_ver2.py 이하 학습 스크립트는 num_node_features 를 데이터에서 동적으로
  읽으므로(node_scale.size(0) 등) 이 파일 쪽 변경만으로 5열을 그대로 받아들인다 — 다만
  plot_preprocessor_check.py 는 x[:, :4] 만 보므로 density 열은 시각화에 안 나타난다(무해).

runtime 우려: density 윈도우(10um)는 게이지 추출 윈도우(0.512um)보다 면적 기준 약 381배
넓어서, 게이지당 1번이라도 총 연산량이 작지 않을 수 있다. 노드별이 아니라 게이지당 1회로
줄인 것 자체가 1차 완화책이고, 그래도 느리면 다음 단계로 좌표를 성긴 격자로 스냅해 인접
게이지끼리 density 를 캐시 재사용하는 방법이 있다(리스트가 셔플/원래도 공간적으로 안 뭉쳐
있어서 워커별 캐시 적중률은 낮을 수 있음 — 필요해지면 그때 격자 셀 크기로 보완).
"""
...
# 밀도 계산 윈도우 한 변 길이(um). 게이지 중심 기준 정사각형.
DENSITY_WINDOW_UM = 10.0
...
def _compute_gauge_density(cell: db.Cell, layer_id: int, cx: float, cy: float,
                            window_um: float, dbu: float) -> float:
    """
    게이지 중심(cx,cy) 기준 window_um x window_um 정사각형 안의 폴리곤 면적비(0~1).
    게이지 1개당 1번만 호출 — 그 게이지의 모든 노드가 이 값 하나를 공유한다.
    Region.area(box) 가 box 로 바로 clip 해서 면적을 주므로 별도 boolean AND 불필요.
    """
    half = window_um / 2
    box = db.Box.from_dbox(_make_search_dbox(cx, cy, half, dbu))
    window_area = box.width() * box.height()
    if window_area <= 0:
        return 0.0
    region = db.Region(cell.begin_shapes_rec_touching(layer_id, box))
    poly_area = region.area(box)
    return float(poly_area) / float(window_area)
...
def _worker_task(args) -> Tuple[str, Optional[dict]]:
    ...
    try:
        ...
        # gauge_density: 이 게이지 1개당 1번만 계산해서 모든 노드에 동일하게 broadcast.
        # x 열이 [cx,cy,w,h] -> [cx,cy,w,h,gauge_density] 로 늘어난다.
        gauge_density = _compute_gauge_density(
            _worker_top_cell, _worker_layer_id, cx, cy, DENSITY_WINDOW_UM, dbu
        )
        node_matrix = np.column_stack([
            node_matrix, np.full(num_nodes, gauge_density, dtype=np.float32)
        ])
